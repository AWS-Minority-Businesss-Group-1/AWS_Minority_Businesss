...
export interface HeaderFilterAlbumPageProps {
  className?: string;
  setShowModal: React.Dispatch<React.SetStateAction<boolean>>;
}

const HeaderFilterAlbumPage: FC<HeaderFilterAlbumPageProps> = ({
  className = "mb-12",
  setShowModal,
}) => {
    ...
    <div className={`flex flex-col relative ${className}`}>
      <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-6 lg:space-y-0 lg:space-x-2 ">
        ...
        <span className="block flex-shrink-0 text-right">
          ...
          <Button
            className="w-auto !pr-16 bg-green-500"
            sizeClass="pl-4 py-2.5 sm:pl-6"
          ...
          >
            ...
            <span className="block truncate ml-2.5 text-white">Add File</span>
            <span className="absolute top-1/2 -translate-y-1/2 right-5">
              {/* <ChevronDownIcon
                className={`w-4 h-4 sm:w-5 sm:h-5 ${
                  isOpen ? "rotate-180" : ""
...
export default HeaderFilterAlbumPage;
